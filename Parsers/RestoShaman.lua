...
addon.BuffTracker:Track(
	addon.Shaman.AscendanceBuff,
	function()
		WeightedAvgStart(ascendance)
	end,
	nil
...
local function _HealEvent(ev, spellInfo, heal, overhealing, destUnit, f, origHeal)
	...
	elseif (addon.BuffTracker:Get(addon.Shaman.AscendanceBuff) > 0) then
		...
		if (spellInfo.mst) then
			...
			ascendance.masteffect_times_heal =
				ascendance.masteffect_times_heal + getMasteryEffect(destUnit, origHeal) * total_heal
		end
...
addon.StatParser:Create(
	addon.SpellType.SHAMAN,
	_Intellect,
	_CriticalStrike,
	_Haste,
	_Versatility,
