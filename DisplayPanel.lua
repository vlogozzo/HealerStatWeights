...
local crit_resurg = " (R)"
local pawn_pattern =
    [[( Pawn: v1: "%s": Class=%s, Spec=%s, Intellect=%.2f, CritRating=%.2f, HasteRating=%.2f, Versatility=%.2f, MasteryRating=%.2f, Leech=%.2f)]]
...
local qelive_pattern =
    "https://questionablyepic.com/live/?import=HSW&spec=%s&pname=%s&realm=%s&region=%s&CriticalStrike=%.2f&HasteHPCT=%.2f&HasteHPM=%.2f&Mastery=%.2f&Versatility=%.2f&Leech=%.2f&content=%s"
local qelive_noname_pattern =
    "https://questionablyepic.com/live/?import=HSW&spec=%s&CriticalStrike=%.2f&HasteHPCT=%.2f&HasteHPM=%.2f&Mastery=%.2f&Versatility=%.2f&Leech=%.2f&content=%s"
...
function addon:UpdateDisplayLabels()
    ...
    self.frame.textL:SetFormattedText(pattern_left, crit_suffix, haste_suffix, vers_suffix, mastery_suffix)
end
...
function addon:SegmentMenu()
    local menu = {
        ...
        {text = "Total", func = function()
                addon:SetCurrentSegment("Total")
            end},
        {text = "Current Fight", func = function()
...
function addon:Enabled()
    if (self.StatParser:IsCurrentSpecSupported()) then
        ...
        if (level >= minLevel) then
            ...
            if
                ((id == 1 and self.hsw.db.global.enabledInNormalDungeons) or
                    ...
                    (id == 17 and self.hsw.db.global.enabledInLfrRaids))
             then
                return true
...
function addon:SetupFrame()
    if (not self.frame) then
        ...
        makeButton(
            frame,
            "Export",
            "Get the pawn string for the current statweights.",
